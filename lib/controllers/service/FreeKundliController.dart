...
class FreeKundliController extends GetxController with GetTickerProviderStateMixin {
    ...
    await horoscopeProvider.fetchKundli(storage.read("access"), "", data).then((response) async {
      if(response.code==1) {
        try {
          ...
          storage.write("kundli_${kundli?.id??-1}", response.data);
        }
        catch(ex) {
  ...
  //     update();
  //
  //     await storage.write("kundli_${kundli?.id??-1}", StorageKundliModel(basic: basic, planet: planet, kpPlanet: kpPlanet, vimshottari: vimshottari, yogini: yogini, charts: charts));
  //   });
  // }
  ...
  String getChartSVG() {
    ...
    if(tabController.index==0) {
      print(CommonConstants.charts[0]);
      print(charts[CommonConstants.charts[0]]);
  ...
  void selectKundli(KundliModel kundli) {
    ...
    if(storage.read("kundli_${kundli.id}")!=null) {
      ...
      try {
        KundliDetailModel kundliDetail = storage.read("kundli_${kundli.id}");
