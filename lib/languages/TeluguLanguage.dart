Map<String, String> telugu = {
  //Languages
  'English' : 'ఆంగ్ల',
  'Hindi' : 'హిందీ',
  'Telugu' : 'తెలుగు',

  'Choose Language' : 'భాషను ఎంచుకోండి',

  //Common
  'Search Astrologer' : 'జ్యోతిష్యుడిని శోధించండి',
  'Read More' : 'ఇంకా చదవండి',
  'Read Less' : 'తక్కువ చదువుతాను',

  //Dashboard
  'Dashboard': 'డాష్బోర్డ్',
  'Free Kundali' : 'ఉచిత కుండ్లి',
  'Match Kundali' : 'కుండ్లిని సరిపోల్చండిి',
  'Daily Horoscope' : 'రోజువారీ జాతకం',
  'Currently Live Astrologer' : 'ప్రస్తుతం ప్రత్యక్ష జ్యోతిష్యుడు',
  'New Arrival Astrologer' : 'కొత్త రాక జ్యోతిష్కుడు',
  'New Updates and Blogs' : 'కొత్త నవీకరణలు మరియు బ్లాగులు',
  'User Testimonials' : 'వినియోగదారు టెస్టిమోనియల్స్',
  'New Videos' : 'కొత్త వీడియోలు',
  'Why AstroGuide?' : 'జఆస్ట్రో గైడ్ ఎందుకు?',
  'Easily connect with Astro' : 'జ్యోతిష్కునితో సులభంగా కనెక్ట్ అవ్వండి',
  'You can easily connect with astrologer with the help of AstroGuide.' : 'మీరు AstroGuide సహాయంతో జ్యోతిష్కునితో సులభంగా కనెక్ట్ కావచ్చు.',
  'Privacy guarantee 100%' : 'గోప్యతా హామీ 100%',
  'AstroGuide provide 100% privacy guarantee to your personal data.' : 'AstroGuide మీ వ్యక్తిగత డేటాకు 100% గోప్యతా హామీని అందిస్తుంది.',
  'Approved Astrologer' : 'ఆమోదించబడిన జ్యోతిష్యుడు',
  'You can talk with 100% verified and approved Astrologer.' : 'మీరు 100% ధృవీకరించబడిన మరియు ఆమోదించబడిన జ్యోతిష్కుడితో మాట్లాడవచ్చు.',
  'View all' : 'అన్నీ చూడండి',
  'Live' : 'ప్రత్యక్ష',
  'min' : 'నిమిషం',

  //Blog
  'Written by' : 'వ్రాసిన వారు',

  //Talk
  'Talk with Astrologer' : 'జ్యోతిష్కుడితో మాట్లాడండి',
  'Reviews' : 'సమీక్షలు',

  //History
  'History' : 'చరిత్ర',
  'Wallet' : 'వాలెట్',
  'Call' : 'కాల్',
  'Chat' : 'చాట్',
  'Available Balance' : 'అందుబాటులో ఉన్న రీఛార్జ్',
  'Recharge' : 'రీఛార్జ్ చేయండి',
  'Wallet Transactions' : 'వాలెట్ లావాదేవీలు',
  'Payment Logs' : 'చెల్లింపు లాగ్‌లు',
  'Session Type' : 'సెషన్ రకం',
  'Rate' : 'ధర',
  'SHOW CHAT' : 'చాట్ చూపించు',

  //Settings
  'Settings' : 'సెట్టింగ్‌లు',
  'Wishlist' : 'ఇష్టమైన',
  'Following' : 'ఫాలోయింగ్',
  'Notification' : 'నోటిఫికేషన్',
  'Appearance' : 'ప్రదర్శన',
  'Change App Language' : 'యాప్ లాంగ్వేజ్ మార్చండి',
  'Terms and Condition' : 'షరతులు మరియు నిబంధనలు',
  'Privacy Policy' : 'గోప్యతా విధానం',
  'About Us' : 'మా గురించి',
  'Contact Us' : 'మమ్మల్ని సంప్రదించండి',
  'Logout' : 'లాగ్అవుట్',

  //Astrologer
  'Astrologer Profile' : 'జ్యోతిష్కుడి ప్రొఫైల్',
  'About Astrologer' : 'జ్యోతిష్యుని గురించి',
  'Languages Spoken' : 'మాట్లాడగల భాషలు',
  'Expertise' : 'నైపుణ్యం',
  'User Reviews' : 'వినియోగదారు సమీక్షలు',
  'View All Reviews' : 'అన్ని సమీక్షలను వీక్షించండి',
  'Total min chat' : 'మొత్తం కనీస చాట్',
  'Total min call' : 'మొత్తం కనీస కాల్',

  //Wallet
  'My Wallet' : 'నా పర్సు',
  'Available Balance' : 'అందుబాటులో ఉన్న బ్యాలెన్స్',
  'Custom Amount' : 'కస్టమ్ మొత్తం',
  'Enter Amount' : 'మొత్తాన్ని నమోదు చేయండి',
  'Recharge Wallet' : 'రీఛార్జ్ వాలెట్',
  'Choose from the available recharge packs.' : 'అందుబాటులో ఉన్న రీఛార్జ్ ప్యాక్‌ల నుండి ఎంచుకోండి.',
  'Proceed to pay' : 'చెల్లించడానికి కొనసాగండి',
  'Extra' : 'మరింత',

  //My Profile
  'My Profile' : 'నా జీవన వివరణ',
  'Update your profile' : 'ప్రొఫైల్‌ను నవీకరించండి',

  //Horoscope
  "Today's Horoscope" : "నేటి రాశిఫలం",
};